...
public class AVL<T extends Comparable<? super T>> {
    ...
    private AVLNode<T> rRemove(AVLNode<T> curr, T data, AVLNode<T> dummy) {
    	if (curr==null) {
    		...
    		if (curr.getLeft() == null && curr.getRight() == null) {
    			...
    			AVLNode<T> successor = getNodeSuccessor(curr);
    			AVLNode<T> dummyS = new AVLNode<T>(null);
    			curr.setData(successor.getData());
    			curr.setRight(rRemove(curr.getRight(), successor.getData(), dummyS));
                if (curr.getRight()!=null){
                    updateHeightAndBF(curr.getRight());
                    if (Math.abs(curr.getRight().getBalanceFactor()) > 1){
    ...
    private AVLNode<T> getNodeSuccessor(AVLNode<T> curr) {
    	curr = curr.getRight();
    ...
    private void updateHeightAndBF(AVLNode<T> currentNode) {
        ...
        if (currentNode.getLeft()==null && currentNode.getRight()==null){
            ...
            currentNode.setHeight(Math.max(-1, currentNode.getRight().getHeight())+1);
            currentNode.setBalanceFactor(-1 - currentNode.getRight().getHeight());
        } else if (currentNode.getLeft()!=null && currentNode.getRight()==null) {
            currentNode.setHeight(Math.max(currentNode.getLeft().getHeight(), -1) + 1);
    ...
    private AVLNode<T> rotateLeft(AVLNode<T> currentNode) {
        // WRITE YOUR CODE HERE (DO NOT MODIFY METHOD HEADER)!
        AVLNode<T> tmpNode = currentNode.getRight();
        currentNode.setRight(tmpNode.getLeft());
        tmpNode.setLeft(currentNode);

        // recalculate the height and balance factor of the affected
        updateHeightAndBF(currentNode);
        updateHeightAndBF(tmpNode);
    ...
    private AVLNode<T> rotateRight(AVLNode<T> currentNode) {
        // WRITE YOUR CODE HERE (DO NOT MODIFY METHOD HEADER)!
        AVLNode<T> tmpNode = currentNode.getLeft();
        currentNode.setLeft(tmpNode.getRight());
        tmpNode.setRight(currentNode);
